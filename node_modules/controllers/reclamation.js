  ...
  reclamations.find({token:token},function(err,data){
    if(data.length != 0){
      ...
      for (i = 0; i < data.length; i++) {
        dataRec.push({ '_id':data[i]._id, 'subject':data[i].subject, 'date':data[i].date, 'status':data[i].status });
      }
...
exports.addReclamation = function(token,username,subject,message,callback){
  ...
  var newReclamation = new reclamations({token:token,username:username,subject:subject,date:date,
    status:false,messages:{sender:true,message:message,date:date}});
...
exports.addMessage = function(idRec,message,callback){
  ...
  reclamations.update({_id:idRec},{$set:{status:false},$push:{messages:{sender:true,message:message,date:date}}},function(err){
    if(err){
      callback({'res':false});
    }else{
