...
exports.signup = function(fname,lname,gender,dateN,country,city,email,password,phone,picture,callback){
  ...
  var newUser = new drivers({fname:fname,lname:lname,gender:gender,dateN:dateN,country:country,city:city,email:email,
    password:hashed_password,phone:phone,pt:0,ptt:1,picture:picture,connected:false,salt:temp });
  drivers.find({email: email},function(err,data){
    if(data.length == 0){
...
exports.getDriver = function(token,callback){
  drivers.find({_id:token},function(err,data){
    if(data.length != 0){
      ...
      var picture = data[0].picture;
      // fixed this
      var color = data[0].taxis[0].color;
      var model = data[0].taxis[0].model;
      var serial = data[0].taxis[0].serial;
      var places = (data[0].taxis[0].places).toString();
      var luggages = (data[0].taxis[0].luggages).toString();
      var keyVitual = method.keyVirtual();
      var key = method.key(keyVitual);
      callback({'res':true,'key':keyVitual,'fname':method.encode(fname,key),'lname':method.encode(lname,key),
        'dateN':method.encode(dateN,key),'pt':method.encode(pt,key),'ptt':method.encode(ptt,key),'picture':picture,
        'color':method.encode(color,key),'model':method.encode(model,key),'serial':method.encode(serial,key),
...
exports.getTaxi = function(token,callback){
  drivers.aggregate({$unwind:'$taxis'},{$match:{_id:token}},{$sort:{'taxis.date':1}},function(err,data){
    if(data.length != null){
      var keyVitual = method.keyVirtual();
      var key = method.key(keyVitual);
			var dataTaxi = [];
      for (i = 0; i < data.length; i++) {
        dataTaxi.push({ '_id':data[i].taxis.idTaxi,'model':method.encode(data[i].taxis.model,key),
          'serial':method.encode(data[i].taxis.serial,key),'places':method.encode((data[i].taxis.places).toString(),key),
          'luggages':method.encode((data[i].taxis.luggages).toString(),key),'color':method.encode(data[i].taxis.color,key),
          'date':method.encode(data[i].taxis.date,key),'working':method.encode(data[i].taxis.working,key) });
      }
			callback({'res':true,'data':dataTaxi});
		}else{
			callback({'res':false});
		}
  });
