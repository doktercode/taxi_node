...
exports.getDriver = function(token,callback){
  drivers.find({_id:token},function(err,data){
    if(data.length != 0){
      ...
      var places = data[0].taxis[0].places;
      var luggages = data[0].taxis[0].luggages;
      var keyVitual = method.keyVirtual();
      var key = method.key(keyVitual);
      callback({'res':true,'key':keyVitual,'fname':method.encode(fname,key),'lname':method.encode(lname,key),
        'dateN':method.encode(dateN,key),'picture':picture,'color':method.encode(color,key),'model':method.encode(model,key),
        'serial':method.encode(serial,key),'places':method.encode(places,key),'luggages':method.encode(luggages,key)});
    }else{
      callback({'res':false});
    }
  });
